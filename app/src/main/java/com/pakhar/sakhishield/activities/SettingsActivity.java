...
public class SettingsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_settings); //loadds xml
        dbHelper = new DatabaseHelper(this);
        prefs    = getSharedPreferences("SakhiShieldPrefs", MODE_PRIVATE); //opens local storage
        userPhone = prefs.getString("userPhone", ""); //retrives current logged in user
        // Bind views
        ...
        loadCurrentProfile();
        loadSwitchStates(); //loads saed rec
        saveProfileBtn.setOnClickListener(v -> saveProfile());
        autoAudioSwitch.setOnCheckedChangeListener(  // Auto audio toggle
                (CompoundButton buttonView, boolean isChecked) -> {
                    prefs.edit().putBoolean(RecordingManager.PREF_AUTO_AUDIO, isChecked)
                            .apply();
                    Toast.makeText(this, "Auto audio " + (isChecked ? "enabled" : "disabled"),
                            Toast.LENGTH_SHORT).show();
                });
        autoVideoSwitch.setOnCheckedChangeListener( //auto vdo toggle
                (CompoundButton buttonView, boolean isChecked) -> {
    ...
    private void loadSwitchStates() {
        ...
        autoAudioSwitch.setChecked(prefs.getBoolean(RecordingManager.PREF_AUTO_AUDIO, true));
        autoVideoSwitch.setChecked(prefs.getBoolean(RecordingManager.PREF_AUTO_VIDEO, false));
    }
